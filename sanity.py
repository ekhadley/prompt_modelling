...
if dijkstra_find_best_replacement_completion_batch:
    true_stok = "▁pirate"
    batch_size = 16        # conversations per batch AND neighbours expanded per node
    n_batches = 1          # how many conversation batches to average the completion loss over
    tok_low, tok_high = 107, 250_000
    ...
    W_E = model.W_E.clone()  # normalized, used only for the embedding-similarity graph
    W_E -= W_E.mean(dim=-1, keepdim=True)
    W_E /= W_E.norm(dim=-1, keepdim=True)
    tec()

    def score_tokens(cand_ids):  # mean completion loss with each candidate placed at the target position, averaged over the conversation batches
        losses = t.zeros(len(cand_ids), device=device)
        for conv_toks, attn_mask, comp_mask, targ_indices in batches:
            ...
            for i, c in enumerate(cand_ids):
                ct = conv_toks.clone()
                ct[t.arange(ct.shape[0]), targ_indices] = c
                logprobs = model.forward(ct, attention_mask=attn_mask).log_softmax(dim=-1)
                tok_losses = -logprobs[batch_indices, seq_indices, ct[batch_indices, seq_indices + 1]]
                comp_losses = tok_losses * comp_mask[:, 1:]
                losses[i] += (comp_losses.sum() / comp_mask.count_nonzero()) / len(batches)
            t.cuda.empty_cache()
        return losses
    ...
    for _ in (bar:=trange(1000, ncols=140, ascii=" >=")):
        ...
        # if true_stok.lower() in top_stok.lower().strip():
        #     break
        ...
        neighborhood = top_tok_sims.topk(batch_size**2).indices.tolist()
        nbrs = []
        for nbr_tok_id in neighborhood:
            if nbr_tok_id >= tok_low and nbr_tok_id < tok_high and nbr_tok_id not in seen:
                nbrs.append(nbr_tok_id)
            if len(nbrs) == batch_size: break
        nbrs = t.tensor(nbrs, device=device)

        scores = score_tokens(nbrs)
